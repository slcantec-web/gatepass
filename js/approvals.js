...
async function renderApprovals(container) {
  ...
  container.innerHTML = `
    ...
    <table class="data-table">
      <thead><tr><th>Pass #</th><th>Leader</th><th>Purpose</th><th>Actions</th></tr></thead>
      <tbody>
        ${pending.map((p) => `
          <tr>
            <td>${p.pass_number}</td><td>${p.leader_name}</td><td>${p.purpose}</td>
            <td>
        ...
        `).join("") || `<tr><td colspan="4">Nothing pending approval.</td></tr>`}
      </tbody>
    </table>
  `;
