...
// EMPLOYEE logins see only their own passes (as leader or member).
// Every other role (SUPER_ADMIN, ADMIN, HOD, SECURITY, MANAGEMENT_VIEWER) gets
// the full gate pass history - every pass, any status, any leader - with the
// leader's name joined in so the list reads as a real history rather than
// just bare IDs. This is what the frontend's "Gate Pass History" view (the
// same screen as "My Gate Passes", it just detects which shape it got back)
// is built on.
...
export async function deleteGatePass(env, user, passId, request) {
  const pass = await env.DB.prepare(`SELECT * FROM gate_passes WHERE pass_id = ?`).bind(passId).first();
  if (!pass) {
    const err = new Error("Pass not found");
    err.status = 404;
